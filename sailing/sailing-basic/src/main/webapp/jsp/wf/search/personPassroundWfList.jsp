...
<script type="text/javascript">
	   ...
	   var Query= function(){
			...
			var urlTemp=document.getElementById("qware").value + "/wf/personWfSearchAction.do?step=passroundList&paginater.page=0"
...
<% 
	//取到SESSION中暂存的值
	//处理查询值回显问题
	String title = session.getAttribute("wfsearch4")== null? "" : (((WfSearch)session.getAttribute("wfsearch4")).getTitle());
	String startTime = session.getAttribute("wfsearch4")== null? "" : (((WfSearch)session.getAttribute("wfsearch4")).getStartTime());
	String endTime = session.getAttribute("wfsearch4")== null? "" : (((WfSearch)session.getAttribute("wfsearch4")).getEndTime());
%>
	<tr>
...
<table border="0" cellpadding="0" cellspacing="0" class="Listing"
	...
	<tbody id='tablist'>
		<c:forEach var="item" items="${theForm.passroundList}" varStatus="status">
			<tr>
				...
				<td align="center"><c:out value="${item.commitTime}" /></td>
...
<div class="list_bottom">
	<c:set var="paginater.forwardUrl" value="/wf/personWfSearchAction.do?step=passroundList" scope="page" /> 
