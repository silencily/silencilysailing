...
<script type="text/javascript">
	   ...
	   var Query= function(){
			...
			var urlTemp=document.getElementById("qware").value + "/wf/personWfSearchAction.do?step=waitList&paginater.page=0"
	   ...
	   CurrentPage.workflowUrl = function(e) {
			...
			//步骤ID
			var stepId = trObj.cells[3].childNodes[1].value;
			//工作流名称
			var wfname = trObj.cells[0].childNodes[1].value;			
			var workflowurl=document.getElementById("myActionurl").value + boxurl.value + '&stepId='+stepId;		
...
<% 
	//取到SESSION中暂存的值
	//处理查询值回显问题
	String title = session.getAttribute("wfsearch")== null? "" : (((WfSearch)session.getAttribute("wfsearch")).getTitle());
	String startTime = session.getAttribute("wfsearch")== null? "" : (((WfSearch)session.getAttribute("wfsearch")).getStartTime());
	String endTime = session.getAttribute("wfsearch")== null? "" : (((WfSearch)session.getAttribute("wfsearch")).getEndTime());
%>
...
<table border="0" cellpadding="0" cellspacing="0" class="Listing"
	...
	<tbody id='tablist'>
		<c:forEach var="item" items="${theForm.waitWfList}" varStatus="status">
			<tr>
				<td nowrap="nowrap" class="showAllMessage"  title='<c:out value="${item.title}" />'> 
					<input type="hidden" value='<c:out  value="${item.wfName}"/>' />
					<c:out value="${item.titleCut}"/>
					&nbsp;</td>
				<td align="center"><c:out value="${item.wfState}" /> 
					<input type="hidden" value='<c:out  value="${item.url}"/>' />&nbsp;</td>
				<td nowrap="nowrap"><c:out value="${item.currentStepName}" />
					<input type="hidden" value='<c:out  value="${item.currentStepName}"/>' />&nbsp;</td>
...
<div class="list_bottom">
	<c:set var="paginater.forwardUrl" value="/wf/personWfSearchAction.do?step=waitList" scope="page" /> 
