<%--
    @version:$Id: roleEntry.jsp,v 1.1 2010/12/10 10:56:27 silencily Exp $
...
<script language="javascript">
  ...
  CurrentPage.creatMenu = function creatFolder() {
	...
	<input type="hidden" name="parentCode" id="parentCode" value="<c:out value = "${theForm.parentCode}"/>"/>
		...
		<div class="main_title"><div>角色管理</div></div>
		    <tr valign="top">
		        ...
		        <td> 
		            ...
		            <script type="text/javascript">
		                var arr = new Array();
		                arr[0] = [" 列表 ", "", "<c:url value='/sm/RoleAction.do?step=list&paginater.page=0&flg=role&parentCode='/>"];
		                arr[1] = [" 详细 ", "", "<c:url value='/sm/RoleAction.do?step=detail&flg=role&parentCode='/>"];
		                arr[2] = [" 权限 ", "", "<c:url value = '/sm/RoleAction.do?step=permissionShow&paginater.pageSize=0'/>" ];
						arr[3] = [" 用户 ", "", "<c:url value='/sm/RoleAction.do?step=userShow&paginater.pageSize=0&parentCode='/>"];
