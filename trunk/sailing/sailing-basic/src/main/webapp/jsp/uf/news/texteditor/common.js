...
function ctlent(event, clickactive) {
	if(postSubmited == false && (event.ctrlKey && event.keyCode == 13) || (event.altKey && event.keyCode == 83) && $('postsubmit')) {
		if(in_array($('postsubmit').name, ['topicsubmit', 'replysubmit', 'editsubmit', 'pmsubmit']) && !validate($('f'))) {
			doane(event);
			return;
		}
		postSubmited = true;
...
function AddText(txt) {
	obj = document.getElementById("posteditor_textarea");//$('f').message;
...
function checkFocus() {
	var obj = typeof wysiwyg == 'undefined' || !wysiwyg ? document.getElementById("posteditor_textarea") : editwin;//('f').message : editwin;
	if(!obj.hasfocus) {
		obj.focus();
	}
}
